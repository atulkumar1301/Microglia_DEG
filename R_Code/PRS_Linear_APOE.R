...
#Clinical_data <- fread (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Microglial_Activation/", args[1]))
Clinical_data <- fread ("/Volumes/ATUL_6TB/Work/Projects/Microglia_DEG/BF_2/5_Data_Full_Imputed_Analysis_Taupet.txt")
Full_File <- fread ("/Volumes/ATUL_6TB/Work/Projects/Microglia_DEG/BF_2/PCA_FILE.eigenvec") %>% merge(Clinical_data, by = 'IID')
...
for (p in colnames (df)) {
  ...
  model_PRS1 <- glm (data2$PHENO ~ data2$NORMSCORE + data2$Age + data2$Gender + data2$PC1 + data2$PC2 + data2$PC3 + data2$PC4 + data2$PC5 + data2$PC6 + data2$PC7 + data2$PC8 + data2$PC9 + data2$PC10, family = gaussian, data = data2)
  ...
  modeldata20 <- glm (data2$PHENO ~ 1, family = gaussian, data = data2)
  model_PRS2 <- glm (data2$PHENO ~ data2$NORMSCORE + data2$Age + data2$Gender + data2$PC1 + data2$PC2 + data2$PC3 + data2$PC4 + data2$PC5 + data2$PC6 + data2$PC7 + data2$PC8 + data2$PC9 + data2$PC10, family = gaussian, data = data2)
  ...
  modeldata20 <- glm (data2$PHENO ~ 1, family = gaussian, data = data2)
  model_PRS3 <- glm (data2$PHENO ~ data2$NORMSCORE + data2$Age + data2$Gender + data2$PC1 + data2$PC2 + data2$PC3 + data2$PC4 + data2$PC5 + data2$PC6 + data2$PC7 + data2$PC8 + data2$PC9 + data2$PC10, family = gaussian, data = data2)
  ...
  modeldata20 <- glm (data2$PHENO ~ 1, family = gaussian, data = data2)
  model_PRS4 <- glm (data2$PHENO ~ data2$NORMSCORE + data2$Age + data2$Gender + data2$PC1 + data2$PC2 + data2$PC3 + data2$PC4 + data2$PC5 + data2$PC6 + data2$PC7 + data2$PC8 + data2$PC9 + data2$PC10, family = gaussian, data = data2)
  ...
  modeldata20 <- glm (data2$PHENO ~ 1, family = gaussian, data = data2)
  model_PRS5 <- glm (data2$PHENO ~ data2$NORMSCORE + data2$Age + data2$Gender + data2$PC1 + data2$PC2 + data2$PC3 + data2$PC4 + data2$PC5 + data2$PC6 + data2$PC7 + data2$PC8 + data2$PC9 + data2$PC10, family = gaussian, data = data2)
  ...
  modeldata20 <- glm (data2$PHENO ~ 1, family = gaussian, data = data2)
  model_PRS6 <- glm (data2$PHENO ~ data2$NORMSCORE + data2$Age + data2$Gender + data2$PC1 + data2$PC2 + data2$PC3 + data2$PC4 + data2$PC5 + data2$PC6 + data2$PC7 + data2$PC8 + data2$PC9 + data2$PC10, family = gaussian, data = data2)
  ...
  #write.table (TABLE, file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Cognitive_Decline/mPACC/PRS_Results_mPACC_APOE.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
  write.table (TABLE, file = paste0 ("PRS_Result_",p,"_APOE.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
}
