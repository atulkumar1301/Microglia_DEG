...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/Microglia_DEG/BF_2/PRS_Plot_APOE_BF-2.txt")
...
p1 <- p1 + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="#F0E442")) +
  geom_hline (aes (yintercept=-log10(0.0014), linetype = "Bonferroni p-value 0.05", col="#999999")) +
...
p1 <- p1 +
  theme(legend.position="right",
        ...
        panel.background = element_blank()) + labs(title=expression("A) Including APOE region variants"*beta))

#### Non-APOE region
df_1 <- fread ("/Volumes/ATUL_6TB/Work/Projects/Microglia_DEG/BF_2/PRS_Plot_Non_APOE_BF-2.txt")
...
annotate_figure(plot, top = text_grob("Cell-Specific PRS association BioFINDER-2", color = "black", face = "bold", size = 16, family = "serif"))
